...
export class Graph {
    ...
    subscribe = (
        ...
        if(nd instanceof GraphNode) {
            sub = nd.__node.listeners.subscribe(callback,key,subInput,target,bound);
           
            let ondelete = () => {
                (nd as GraphNode).__node.listeners.unsubscribe(sub,key,subInput);
            }
            ...
            if(this.get(node)) {
                if(callback instanceof GraphNode && callback.__operator) {
                    sub = (this.get(node) as GraphNode).__node.listeners.__node.listeners.subscribe(callback.__operator,key,subInput,target,bound); 
                    let ondelete = () => {
                        this.get(node).__node.listeners.unsubscribe(sub)
                        //console.log('unsubscribed', key)
                    }
                    ...
                    sub = (this.get(node) as GraphNode).__node.listeners.subscribe(callback,key,subInput,target,bound); 
    ...
    unsubscribe = ( node:GraphNode|string, sub?:number, key?:string, subInput?:boolean) => {
        if(node instanceof GraphNode) {
            return node.__node.listeners.unsubscribe(sub,key,subInput);
        }
        else return this.get(node)?.__node.listeners.unsubscribe(sub,key,subInput);
    }
