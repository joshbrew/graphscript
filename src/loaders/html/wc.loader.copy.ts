...
export const wchtmlloader = (
    ...
    if((node.tagName || node.__element) && !node.__props && !node.__template) {
        ...
        node.__template = `<style>\n${node.__css}\n</style>${node.__template}`; delete node.__css;
    }
    

    const parentNode = properties.parentNode // Ensure this remains static
    ...
    if (node.__template) {
        ...
        const tagName = node.tagName // + Math.floor(Math.random()*1000000000000000);

        // console.error('Registering', node.__node.tag)
        CustomElement.addElement(tagName);

        node.__props = document.createElement(tagName);
    ...
    if(node.__props instanceof HTMLElement) {
        ...
        node.__addOnconnected((n) => { 
            ...
            if(parentNode) {
                if(typeof parentNode === 'string' && document.getElementById(parentNode))  
                    document.getElementById(parentNode)?.appendChild(n.__props);
                else if (parentNode instanceof HTMLElement) parentNode.appendChild(n.__props);
            } else if(parent.__props instanceof HTMLElement) {
                parent.__props.appendChild(node.__props);
            } else if (typeof graph.parentNode === 'string' && document.getElementById(parentNode)) {  
                document.getElementById(parentNode)?.appendChild(graph.__props);
