...
*/
    ...
    */
    ...
    LGraphNode.prototype.connect = function(slot, target_node, target_slot) {
        ...
        //MODDED
		//connect in input
        if(target_node.inputs[target_slot].name === execPin && target_node.inputs[target_slot].link) {
            let i = 0;
    ...
    LGraphNode.prototype.disconnectOutput = function(slot, target_node) {
        this.console.log('disconnect', slot, target_node);
        if (slot.constructor === String) {
            slot = this.findOutputSlot(slot);
            if (slot == -1) {
                if (LiteGraph.debug) {
...
*/
	...
	*/
        ...
        if (this.graph) {
            ...
            if (this.connecting_pos != null) {
                ...
                if (
                    ...
                    ctx.beginPath();
                    ctx.moveTo(this.connecting_pos[0] - 5 + 0.5, this.connecting_pos[1] - 6 + 0.5); // top-left point
                    ctx.lineTo(this.connecting_pos[0] + 5.5, this.connecting_pos[1] + 0.5); // middle-right point
                    ctx.lineTo(this.connecting_pos[0] - 5 + 0.5, this.connecting_pos[1] + 6 + 0.5); // bottom-left point
                    ctx.closePath();
                    ctx.stroke();
                    ...
                    ctx.beginPath();
                    ctx.moveTo(this.graph_mouse[0] - 5 + 0.5, this.graph_mouse[1] - 6 + 0.5); // top-left point
                    ctx.lineTo(this.graph_mouse[0] + 5.5, this.graph_mouse[1] + 0.5); // middle-right point
                    ctx.lineTo(this.graph_mouse[0] - 5 + 0.5, this.graph_mouse[1] + 6 + 0.5); // bottom-left point
                    ctx.closePath();
                    ctx.stroke();
                    // ctx.rect(
    ...
    LGraphCanvas.prototype.drawNode = function(node, ctx) {
        ...
        if (!node.flags.collapsed) {
                ...
                for (var i = 0; i < node.inputs.length; i++) {
                    ...
                    if (
                        ...
                        if(slot.link == null) {
                            if (horizontal) {
                                ctx.beginPath();
                                ctx.moveTo(pos[0] - 5 + 0.5, pos[1] - 2 + 0.5); // top-left point
                                ctx.lineTo(pos[0] + 5.5, pos[1] + 0.5); // middle point
                                ctx.lineTo(pos[0] - 5 + 0.5, pos[1] + 2 + 0.5); // bottom-left point
                                ctx.closePath();
                                ctx.stroke();
                            } else {
                                ctx.beginPath();
                                ctx.moveTo(pos[0] - 5 + 0.5, pos[1] - 2 + 0.5); // top-left point
                                ctx.lineTo(pos[0] + 5.5, pos[1] + 0.5); // middle-right point
                                ctx.lineTo(pos[0] - 5 + 0.5, pos[1] + 2 + 0.5); // bottom-left point
                                ctx.closePath();
                                ctx.stroke();
                            }
                            ...
                            if (horizontal) {
                                ctx.beginPath();
                                ctx.moveTo(pos[0] - 5 + 0.5, pos[1] - 6 + 0.5); // top-left point
                                ctx.lineTo(pos[0] + 5.5, pos[1] + 0.5); // middle point
                                ctx.lineTo(pos[0] - 5 + 0.5, pos[1] + 6 + 0.5); // bottom-left point
                                ctx.closePath();
                                ctx.stroke();
                            } else {
                                ctx.beginPath();
                    ...
                    //render name
                    if (render_text && !(
                        slot_type === LiteGraph.EVENT || //MODDED
            ...
            if (output_slot) {
                ...
                ctx.beginPath();
                if (
                    slot.type === LiteGraph.EVENT || //MODDED
                    slot.shape === LiteGraph.BOX_SHAPE
                ) {
                    ctx.rect(x - 7 + 0.5, y - 4, 14, 8);
                } else if (slot.shape === LiteGraph.ARROW_SHAPE) {
    ...
    LGraphCanvas.prototype.drawConnections = function(ctx) {
        ...
        for (var n = 0, l = nodes.length; n < l; ++n) {
            ...
            for (var i = 0; i < node.inputs.length; ++i) {
                ...
                if(input?.extraLinks) {
                    for(const key in input.extraLinks) {
                        let l = this.graph.links[input.extraLinks[key]];
                        drawLink(l);
                    }
                }
    ...
    LGraphCanvas.prototype.renderLink = function(
        ...
        //choose color
        if(inputSlot?.name === execPin) { //MODDED
            color = '#FFF';
        } else if(!link || link.type === "any") color = '#EAD6FF'
        if (!color && link) {
        ...
        if (
            ...
            //render arrow
            if (this.render_connection_arrows || inputSlot?.name === execPin) { //MODDED
                ...
                var angleA = 0;
                var angleB = 0;
                if (this.render_curved_connections === false) { //MODDED
            ...
            if(inputSlot?.name === execPin) {//MODDED
                ...
                var posE = this.computeConnectionPoint(
                    a,
                    b,
                    0.75,
                    start_dir,
                ...
                var posF = this.computeConnectionPoint(
                    a,
                    b,
                    0.76,
                    start_dir,
                ...
                let angleC = -Math.atan2(posF[0] - posE[0], posF[1] - posE[1]);
                ctx.save();
    ...
    LGraphCanvas.prototype.showLinkMenu = function(link, e) {
 	...
 	LGraphCanvas.prototype.createDefaultNodeForSlot = function(optPass) { // addNodeMenu for connection
		...
		if(slotTypesDefault && slotTypesDefault[fromSlotType]){
			...
			nodeNewType = false;
			if(slotTypesDefault[fromSlotType] == "object" || slotTypesDefault[fromSlotType] == "array"){
