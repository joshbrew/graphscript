(() => {
  ...
  var GraphNode = class {
    constructor(properties = {}, parentNode, graph) {
      ...
      this.subscribe = (callback, tag2 = this.tag) => {
        if (callback instanceof GraphNode) {
          return this.subscribeNode(callback);
        } else
          return this.state.subscribeTrigger(tag2, callback);
      };
      this.unsubscribe = (sub, tag2 = this.tag) => {
      ...
      if (typeof properties === "object") {
        if (properties instanceof GraphNode && properties.initial)
          Object.assign(properties, properties.initial);
        if (properties instanceof Graph) {
          ...
          if (source.node) {
            if (source.node.initial)
              Object.assign(properties, source.node.initial);
          }
          if (source.initial)
            Object.assign(properties, source.initial);
        ...
        if (properties.children)
          this.initial.children = Object.assign({}, properties.children);
    ...
    constructor(tree2, tag2, props) {
      ...
      this.subscribeNode = (inputNode, outputNode) => {
        ...
        if (inputNode?.tag)
          ...
          tag2 = inputNode;
        return this.state.subscribeTrigger(tag2, (res) => {
...
"${n.tag}:${n.print(n, printChildren)}"`;
      ...
      this.tag = tag2 ? tag2 : `graph${Math.floor(Math.random() * 1e11)}`;
      if (props) {
  ...
  var Service = class extends Graph {
    constructor(options = {}) {
      ...
      this.keepState = true;
      this.load = (routes2, includeClassName = true, routeFormat = ".") => {
        ...
        if (!(routes2 instanceof Graph) && routes2?.name) {
          if (routes2.module) {
            ...
            Object.getOwnPropertyNames(routes2.module).forEach((prop) => {
              if (includeClassName)
                routes2[mod.name + routeFormat + prop] = routes2.module[prop];
              else
        ...
        } else if (routes2 instanceof Graph || routes2.source instanceof Graph) {
          service = routes2;
          routes2 = {};
          let name;
          if (includeClassName) {
            ...
            let checkChildGraphNodes = (nd, prev) => {
              if (!checked[nd.tag] || prev && includeClassName && !checked[prev?.tag + routeFormat + nd.tag]) {
                ...
                if (nd instanceof Graph || nd.source instanceof Graph) {
                  ...
                  nd.nodes.forEach((n) => {
                    if (includeClassName)
                      routes2[nd.tag + routeFormat + n.tag] = n;
                    else if (!routes2[n.tag])
                      routes2[n.tag] = n;
          ...
          if (name && name !== "Object") {
            ...
            Object.getOwnPropertyNames(module).forEach((route) => {
              if (includeClassName)
                routes2[name + routeFormat + route] = module[route];
              else
        ...
        if (service instanceof Graph && service.name && includeClassName) {
          ...
          for (const prop in routes2) {
            ...
            delete routes2[prop];
            routes2[service.name + routeFormat + prop] = route;
          }
        }
        ...
        for (const prop in this.routes) {
          if (this.routes[prop]?.aliases) {
            ...
            aliases.forEach((a) => {
              if (service)
                routes2[service.name + routeFormat + a] = this.routes[prop];
              else
  ...
  var DOMElement = class extends HTMLElement {
    ...
    static addElement(tag2 = this.tag, cls = this, extend = void 0) {
      addCustomElement(cls, tag2, extend);
    }
  ...
  function addCustomElement(cls, tag2, extend = null) {
    try {
      if (extend) {
        if (tag2)
          window.customElements.define(tag2, cls, { extends: extend });
        else
          window.customElements.define(cls.name.toLowerCase() + "-", cls, { extends: extend });
      } else {
        if (tag2)
          window.customElements.define(tag2, cls);
        else
  ...
  var DOMService = class extends Graph {
    constructor(options, parentNode) {
      ...
      this.load = (routes2, includeClassName = true, routeFormat = ".") => {
        ...
        routes2 = Object.assign({}, routes2);
        for (const route in routes2) {
          if (typeof routes2[route] === "object" && !(routes2[route] instanceof GraphNode)) {
            let r = routes2[route];
            if (typeof r === "object") {
              if (r.template) {
                if (!routes2[route].tag)
                  routes2[route].tag = route;
                this.addComponent(routes2[route], routes2[route].generateChildElementNodes);
              } else if (r.context) {
                if (!routes2[route].tag)
                  routes2[route].tag = route;
                ...
                if (!routes2[route].tag)
                  routes2[route].tag = route;
                this.addElement(routes2[route], routes2[route].generateChildElementNodes);
              }
              if (r.get) {
                if (typeof r.get == "object") {
                }
  ...
  var expected = (input2 = []) => operators.multiply(operators.subtract(operators.add(input2[0], input2[1])));
