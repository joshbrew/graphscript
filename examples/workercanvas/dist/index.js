...
"${n2.tag}:${n2.print(n2, printChildren)}"`;
  ...
  var Systems = {
    collision: {
      tag: "collision",
      setupEntities: function(entities) {
    ...
    nbody: {
      ...
      attract: function(body1, body2, dist, G = this.G, vecn) {
        if (dist === void 0)
          dist = Systems.collision.distance3D(body1.position, body2.position);
        if (vecn === void 0)
          vecn = Systems.collision.normalize(
    ...
    boid: {
      tag: "boid",
      lastTime: performance.now(),
      defaultAnchor: { x: Math.random(), y: Math.random(), z: Math.random(), mul: 6e-3 },
      setupEntities: function(entities) {
        for (const key in entities) {
          const entity = entities[key];
          this.setupEntity(entity);
      ...
      setupEntity: function(entity) {
        ...
        let boidDefaults = {
          ...
          attractor: Object.assign(adjustedAnchor, { mul: 2e-3 }),
          useCohesion: true,
          useSeparation: true,
          useAlignment: true,
          useSwirl: true,
              ...
              for (let j = 0; j < keys.length; j++) {
                ...
                if (k === w || randj === keys[i] || inRange.indexOf(randj) > -1) {
                  ...
                  let disttemp = Math.sqrt(
                    (p0.position.x - pr.position.x) * (p0.position.x - pr.position.x) + (p0.position.y - pr.position.y) * (p0.position.y - pr.position.y) + (p0.position.z - pr.position.z) * (p0.position.z - pr.position.z)
                  );
  ...
  // ../../services/struct/datastructures/DataStructures.ts
