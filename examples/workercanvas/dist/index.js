(() => {
  ...
  // ../../core/loaders/props/props.loader.ts
  var props_loader_default = (node) => {
    if (node.__properties.__props)
      node.____apply(node.__properties.__props, true);
  };

  // ../../core/libraries/esmonitor/dist/index.esm.js
  ...
  var _pollingId, _sps, _a;
  var Poller = (_a = class {
    constructor(listeners2, sps) {
      __privateAdd(this, _pollingId, void 0);
      __privateAdd(this, _sps, void 0);
      this.listeners = {};
      ...
      this.poll = (listeners22) => {
        iterateSymbols(listeners22, (sym, o) => {
          ...
          if (!isSame(current, history)) {
            ...
            if (typeof current === "object") {
              ...
              listeners22[sym].history = current;
          }
        });
      };
      this.start = (listeners22 = this.listeners) => {
        if (!this.sps)
          this.sps = defaultSamplingRate;
        else if (!__privateGet(this, _pollingId)) {
          console.warn("[escode]: Starting Polling!");
          __privateSet(this, _pollingId, setInterval(() => this.poll(listeners22), 1e3 / this.sps));
        }
      };
      this.stop = () => {
        if (__privateGet(this, _pollingId)) {
          console.warn("[escode]: Stopped Polling!");
          clearInterval(__privateGet(this, _pollingId));
          __privateSet(this, _pollingId, void 0);
        }
      };
      if (listeners2)
    ...
    get sps() {
      return __privateGet(this, _sps);
    }
    set sps(sps) {
      __privateSet(this, _sps, sps);
  ...
  }, _pollingId = new WeakMap(), _sps = new WeakMap(), _a);
  ...
  function define2(key, registerAsNewKey) {
  ...
  var Inspectable = class {
    constructor(target = {}, opts = {}, name, parent) {
      ...
      this.create = (key, parent2, val, set2 = false) => {
        const create = this.check(parent2, key, val);
        if (val === void 0)
          val = parent2[key];
        if (create && !(create instanceof Error)) {
          parent2[key] = new Inspectable(val, this.options, key, this);
          return parent2[key];
        }
        if (set2) {
          try {
            this.proxy[key] = val ?? parent2[key];
          } catch (e) {
            const isESM = esm(parent2);
  ...
  var Monitor = class {
    constructor(opts = {}) {
      ...
      this.set = (path, value, opts2 = {}) => {
        const optsCopy = { ...opts2 };
  ...
  // ../../core/libraries/edgelord/index.ts
  ...
  var globalTo = {};
  var globalActive = {};
  var _triggers, _queue, _toResolveWith, _initialize, _getAbsolutePath, _getPathInfo;
  var Edgelord = class {
    constructor(listeners2, root, context) {
      ...
      this.rootPath = "";
      this.status = "";
      __privateAdd(this, _triggers, []);
      __privateAdd(this, _queue, []);
      __privateAdd(this, _toResolveWith, void 0);
      this.setInitialProperties = (listeners2 = {}, root, context = {}) => {
        ...
        __privateSet(this, _toResolveWith, this.getManager());
      ...
      this.onStart = (f) => {
        const res = __privateGet(this, _toResolveWith);
        const isSame2 = res === this;
        if (isSame2) {
          if (this.status === initializedStatus)
            f();
          else
            __privateGet(this, _queue).push(f);
        } else
          res.onStart(f);
      };
        ...
        for (const first in listeners2) {
          ...
          if (second && typeof second === "object") {
            ...
            for (let fromPath in from) {
              callback(fromPath, to, from[fromPath]);
            }
          } else {
            const from = first;
            const to = second;
      ...
      __privateAdd(this, _initialize, (o) => {
      ...
      this.initialize = (o) => {
        if (!this.status)
          __privateGet(this, _triggers).push(o);
        else if (this.status === registeredStatus) {
          this.status = initializedStatus;
          __privateGet(this, _triggers).forEach(__privateGet(this, _initialize));
          __privateGet(this, _queue).forEach((f) => f());
          __privateSet(this, _queue, []);
          __privateSet(this, _triggers, []);
        } else
          __privateGet(this, _initialize).call(this, o);
      };
      this.start = () => {
        this.register();
        this.initialize();
      };
      __privateAdd(this, _getAbsolutePath, (name) => {
        const sep = this.context.monitor.options.keySeparator;
        return !name || !this.rootPath || this.rootPath === name.slice(0, this.rootPath.length) && name[this.rootPath.length] === sep ? name : [this.rootPath, name].join(sep);
      });
      __privateAdd(this, _getPathInfo, (path) => {
        const output = {
          absolute: {},
          relative: {}
        };
        path = __privateGet(this, _getAbsolutePath).call(this, path);
        ...
        let obj = this.context.monitor.get(output.relative.array, void 0, this.context.instance, false);
      ...
      this.add = (from, to, value = true, subscription) => {
        ...
        if (!value)
          return;
        const fromInfo = __privateGet(this, _getPathInfo).call(this, from);
        const toInfo = __privateGet(this, _getPathInfo).call(this, to);
        ...
        if (typeof value == "string")
          value = toInfo.absolute.array.slice(1).join(this.context.options.keySeparator);
        const info2 = {
          value,
          [listenerObject]: true
        };
        ...
        if (args)
          __privateGet(this, _toResolveWith).initialize({
      ...
      this.addToGlobalLog = (path, mode = "from") => {
        const absolutePath = __privateGet(this, _getAbsolutePath).call(this, path);
        let target = mode === "to" ? this.globals.to : this.globals.from;
      ...
      this.clear = (name) => {
        const value = __privateGet(this, _getAbsolutePath).call(this, name);
        Object.keys(this.active).forEach((from) => {
          Object.keys(this.active[from]).forEach((to) => {
            if (!value || from.slice(0, value.length) === value || to.slice(0, value.length) === value)
              this.remove(from, to);
          });
        });
      };
        ...
        if (isValidInput && update !== void 0) {
          ...
          if (target === toSet) {
            ...
            if (info3.value)
              ...
              console.error(`Cannot set value on ${parentPath.filter((str) => typeof str !== "symbol").join(this.context.options.keySeparator)} from ${from}`);
  ...
  _queue = new WeakMap();
  _toResolveWith = new WeakMap();
  _initialize = new WeakMap();
  _getAbsolutePath = new WeakMap();
  _getPathInfo = new WeakMap();
  var edgelord_default = Edgelord;

  // ../../core/Graph2.ts
  function applyLoader(node, parent, graph = this, tree = graph.__node.tree, properties, tag = node.__node.tag, loader) {
    const args = [node, parent, graph, tree, properties, tag];
    if (typeof loader === "object") {
      if (loader.init)
        loader(...args);
      if (loader.connected)
        node.__addOnconnected(loader.connect);
      if (loader.disconnected)
        node.__addOndisconnected(loader.disconnect);
    } else if (typeof loader === "function")
      loader(...args);
  }
  function applyLoaders(node, parent, graph = this, tree = graph.__node.tree, properties, tag = node.__node.tag, loaders2 = this.__node.loaders) {
    for (const l in loaders2)
      applyLoader(node, parent, graph, tree, properties, tag, loaders2[l]);
  }
  var _parent, _graph, _properties, _options, _applied, _applySetters;
  var GraphNode = class {
    constructor(properties, parent, graph, options) {
      this.__ = Symbol("graphscript");
      this.__node = {
      ...
      __privateAdd(this, _applied, []);
      __privateAdd(this, _applySetters, (properties = __privateGet(this, _properties), proxy = __privateGet(this, _properties), ignore = []) => {
        const isProxy2 = proxy === true;
        ...
        for (const key of keys) {
          if (ignore.includes(key))
            continue;
          if (__privateGet(this, _applied).includes(key))
            continue;
          __privateGet(this, _applied).push(key);
          if (key === "__properties")
            Object.defineProperty(this, key, {
              get: () => __privateGet(this, _properties),
              enumerable: true
            });
          else if (key === "__props") {
            Object.defineProperty(this, key, {
              ...
              set: (value) => {
                proxy[key] = value;
                __privateGet(this, _applySetters).call(this, value, true);
              },
              enumerable: true,
              configurable: true
            });
      ...
      this.____apply = __privateGet(this, _applySetters);
      this.__init = (properties, parent = __privateGet(this, _parent), graph = __privateGet(this, _graph), options = __privateGet(this, _options)) => {
        ...
        if (properties) {
          ...
          if (typeof properties === "object") {
            ...
            __privateGet(this, _applySetters).call(this);
            applyLoaders.call(graph, this, parent, graph, graph?.__node?.tree, properties);
            __privateGet(this, _applySetters).call(this, void 0, void 0, ["__props"]);
            ...
            if (graph) {
              ...
              this.__node.flow.setInitialProperties(this.__listeners, void 0, {
                id: symbol,
                instance: this,
                monitor,
                graph,
              ...
              console.error("No flow manager created for " + this.__node.tag);
          }
        }
      };
  ...
  var _Graph = class {
    constructor(options) {
      ...
      __privateAdd(this, _init, (properties) => {
      ...
      this.setTree = (tree) => {
        ...
        if (cpy.__node)
          delete cpy.__node;
        let listeners2 = this.recursiveSet(cpy, this, void 0, tree);
        if (!tree.__node.tag)
          tree.__node.tag = this.__node.tag;
        const node = __privateGet(this, _init).call(this, tree);
        const children = node.__children;
        const copy = Object.assign({}, children);
        if (children)
          listeners2 = this.recursiveSet(copy, this, void 0, children);
        if (node.__listeners)
          listeners2[node.__node.tag] = node.__listeners;
        this.__node.nodes.forEach((n) => n.__node.flow.start());
        node.__callConnected();
        return cpy;
      };
      this.setLoaders = (loaders2, replace) => {
        console.error("Setting loaders", loaders2);
        if (replace)
          this.__node.loaders = loaders2;
        else {
          if (Array.isArray(this.__node.loaders)) {
            if (Array.isArray(loaders2))
              this.__node.loaders = [...this.__node.loaders, ...loaders2];
            else
              this.__node.loaders = [...this.__node.loaders, ...Object.values(loaders2)];
          } else
      ...
      this.add = (properties, parent) => {
        ...
        if (typeof properties === "object" && (!properties?.__node?.tag || !this.get(properties.__node.tag))) {
          ...
          if (node.__children) {
            const children = node.__children;
            const copy = Object.assign({}, children);
            this.recursiveSet(copy, node, listeners2, children);
          }
          node.__callConnected();
          return node;
        }
      ...
      this.recursiveSet = (t, parent, listeners2 = {}, origin = t) => {
        ...
        for (const key of keys) {
          ...
          if (typeof p === "object") {
            ...
            let parentNode = parent instanceof _Graph ? parent.__node.ref : parent;
      ...
      this.remove = (node) => {
        ...
        if (node instanceof GraphNode) {
          ...
          const recursiveRemove = (t) => {
            for (const key in t) {
              const node2 = t[key].__node.ref;
              this.clearListeners(node2);
              this.delete(node2.__node.tag);
              delete this.__node.tree[node2.__node.tag];
              this.delete(key);
              delete this.__node.tree[key];
              node2.__node.tag = node2.__node.tag.substring(node2.__node.tag.lastIndexOf(".") + 1);
              this.clearListeners(node2);
              node2.__callDisconnected();
              if (node2.__children)
                recursiveRemove(node2.__children);
            }
          };
          if (node.__children)
      ...
      this.get = (tag, base) => {
        if (base instanceof GraphNode)
          base = base.__node.tag;
        if (tag === this.__node.tag)
          return this.__node.ref;
        if (base) {
          const relFrom = [base, tag].join(".");
      ...
      this.subscribe = (from, to, value = true, bound = this.__node.ref) => {
        if (typeof from !== "string")
          from = from.__node.tag;
        if (typeof bound === "string")
          bound = this.get(bound);
        if (bound)
          bound.__node.flow.add(from, to, value);
      };
      this.unsubscribe = (node, from, to) => this.clear(from, to, node);
      this.clear = (from, to, bound) => {
        const nd = typeof bound === "string" ? this.get(bound) : bound;
        if (typeof from === "symbol")
          return this.__node.flow.remove(from);
      ...
      this.activate = (from, value) => this.__node.ref.__node.flow.activate(from, value);
      this.setState = (update) => {
        for (let key in update)
          this.activate(key, update);
      };
      this.init(options);
    }
  };
  ...
  var animate = (node, parent, graph) => {
    if (node.__node.animate === true || node.__animation) {
      ...
      node.__operator = animate2;
      node.__addOnconnected((node2) => {
        if (node2.__node.animating || (!("animating" in node2.__node) || node2.__node.animating) && node2.__animation)
          setTimeout(() => {
            requestAnimationFrame(node2.__operator);
          }, 10);
      });
      node.__addOndisconnected((node2) => {
  ...
  // ../../core/loaders/html/DOMElement.js
  ...
  // ../../core/loaders/html/html.loader.ts
  var htmlloader = (node, parent, graph, tree, properties, key) => {
    ...
    if (node.__onrender) {
      ...
      node.__onrender = (element) => onrender.call(node, element);
    }
    ...
    if (node.__props instanceof HTMLElement) {
      ...
      node.__addOnconnected((n) => {
        ...
        if (n.parentNode) {
          ...
          else if (n.parentNode instanceof HTMLElement)
            n.parentNode.appendChild(n.__props);
  ...
  function initProxyElement(element, worker, id) {
    ...
    for (const [eventName, handler2] of entries) {
      element.addEventListener(eventName, function(event) {
        handler2(event, sendEvent);
      });
    }
    const sendSize = () => {
  ...
  var EventDispatcher = class {
    addEventListener(type, listener) {
      if (this.__listeners === void 0)
        this.__listeners = {};
      const listeners2 = this.__listeners;
    ...
    "initSubprocesses": async function initSubprocesses(subprocesses, service) {
      ...
      for (const p in subprocesses) {
        ...
        if (typeof s.subprocess === "object") {
          ...
          s.worker.post("addSubprocessTemplate", [
            p2.name,
            p2.structs,
            p2.oncreate,
            p2.ondata,
        ...
        if (s.pipeTo) {
          ...
          service.transferFunction(w, function pipeResults(data) {
            let inp = data;
          ...
          w.run("setValue", ["routeProxy", s.route]);
          service.transferFunction(w, function routeProxy(data) {
            let r;
            if (this.__node.graph.otherArgsProxy)
