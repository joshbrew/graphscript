        ...
        if (this.animate && !this.isAnimating && "requestAnimationFrame" in window) {
          ...
          let anim = async () => {
            if (this.isAnimating) {
              if (this.DEBUGNODE)
                console.time(this.tag);
              let result = this.animation(...args);
              if (result instanceof Promise) {
                result = await result;
              }
              if (this.DEBUGNODE) {
...
"${n2.tag}:${n2.print(n2, printChildren)}"`;
  ...
  var Systems = {
    collision: {
      ...
      operator: function(entities) {
        for (const key in entities) {
          const entity1 = entities[key];
          if (entity1.components) {
            if (!entity1.components[this.tag] || !entity1.collisionEnabled)
              continue;
          }
          ...
          for (const key2 in entities) {
            ...
            if (entity2.components) {
              if (!entity2.components[this.tag])
                continue;
            }
      ...
      operator: function(entities) {
        for (const key in entities) {
          ...
          if (entity1.useBoundingBox)
            this.checkBoundingBox(this, entity1);
          if (!entity1.collisionEnabled)
            continue;
          ...
          for (const i in entities) {
            ...
            nested:
              for (const j in entities) {
                ...
                if (k === w || entities[randj].tag === entities[i].tag || inRange.indexOf(randj) > -1) {
                } else {
                  ...
                  if (disttemp > p0.boid.groupRadius) {
                    ...
                    if (p0.boid.useSeparation || p0.boid.useAlignment) {
                      ...
                      if (distInv > p.maxSpeed)
                        distInv = p.maxSpeed;
                      else if (distInv < -p.maxSpeed)
                        distInv = -p.maxSpeed;
                    }
        ...
        "canvas": {
          ...
          onrender: (elm, info) => {
            ...
            if (renderer) {
              ...
              const controls = router.run("transferCanvas", renderer.worker, {
                canvas: elm,
                context: "2d",
                _id: elm.id,
