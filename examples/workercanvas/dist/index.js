(() => {
  ...
  var DOMService = class extends Service {
    constructor(options, parentNode, interpreters) {
      ...
      this.domloader = {
        "dom": (r, parent, graph, tree, props, key) => {
          if ((parent.__node.tag || parent.id) && (parent.template || parent.context || parent.tagName || parent.element) && (r.template || r.context || r.tagName || r.element) && !r.parentNode) {
            if (parent.__node.tag)
              r.parentNode = parent.__node.tag;
            if (parent.id)
              r.parentNode = parent.id;
          }
      ...
      this.addElement = (options, generateChildElementNodes = false) => {
        ...
        if (!options.element)
          options.element = elm;
        if (!options.__operator)
          options.__operator = function(props) {
      ...
      this.updateOptions = (options, element) => {
        if (!options.id && options.__node.tag)
          options.id = options.__node.tag;
        if (!options.__node.tag && options.id)
          options.__node.tag = options.id;
        if (!options.id)
      ...
      this.resolveGraphNode = (element, options) => {
        ...
        if (this.__node.nodes.get(options.id)?.element?.parentNode?.id === options.parentNode || this.__node.nodes.get(options.id)?.parentNode === options.parentNode) {
          ...
          let parent;
          if (parentId)
            parent = this.__node.nodes.get(parentId);
          node = new GraphNode(
      ...
      this.addComponent = (options, generateChildElementNodes = true) => {
        ...
        if (!options.element)
          options.element = elm;
        if (!options.__operator)
          options.__operator = function op(props) {
            if (typeof props === "object")
              for (const key in props) {
                if (this.element) {
                  if (typeof this.element[key] === "function" && typeof props[key] !== "function") {
      ...
      this.terminate = (element) => {
        if (typeof element === "object") {
          ...
          if (this.components[element].node.__node.isAnimating)
            this.components[element].node.__node.isAnimating = false;
        ...
        if (element) {
          if (this.__node.nodes.get(element.id)) {
