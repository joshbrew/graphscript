        ...
        "workers": (node, parent, graph, tree) => {
          ...
          if (rt?.worker || rt?.workerId || rt?.workerUrl) {
            ...
            if (typeof rt.__parent === "string" && tree[rt.__parent]?.worker) {
              tree[rt.__parent].worker.subscribe(rt.parentRoute, rt.__node.tag, rt.blocking);
            } else if (rt.__parent?.worker) {
              rt.__parent.worker.subscribe(rt.parentRoute, rt.__node.tag, rt.blocking);
            }
          }
          return rt;
        }
