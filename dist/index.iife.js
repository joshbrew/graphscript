  ...
  var GraphNode = class {
    constructor(properties = {}, parentNode, graph) {
              ...
              if (Array.isArray(node2.children)) {
                node2.children.forEach((c) => {
                  ...
                  if (c.tag) {
                    ...
                    if (this[c.tag] instanceof GraphNode)
                      delete this[c.tag];
                  }
                  this.nodes.forEach((n) => {
                    if (n.nodes.get(c.tag))
                      n.nodes.delete(c.tag);
                    if (n[c.tag] instanceof GraphNode)
                      delete n[c.tag];
                  });
                  recursivelyRemove(c);
                });
                ...
                if (node2.tag) {
                  ...
                  if (this[node2.tag] instanceof GraphNode)
                    delete this[node2.tag];
                }
                this.nodes.forEach((n) => {
                  if (n.nodes.get(node2.tag))
                    n.nodes.delete(node2.tag);
                  if (n[node2.tag] instanceof GraphNode)
                    delete n[node2.tag];
                });
      ...
      this.convertChildrenToNodes = (n) => {
        let checkParentHasChildMapped = (node, child) => {
          if (node.parent instanceof GraphNode || node.parent instanceof Graph) {
            if (!node.parent.nodes.get(child.tag))
        ...
        if (n?.children) {
          if (n.children instanceof GraphNode) {
            if (!n.graph?.nodes.get(n.tag))
              n.graph.nodes.set(n.tag, n);
            if (!n.nodes.get(n.tag))
            ...
            for (let i = 0; i < n.children.length; i++) {
              if (n.children[i] instanceof GraphNode) {
                if (!n.graph?.nodes.get(n.children[i].tag))
                  n.graph.nodes.set(n.children[i].tag, n.children[i]);
                if (!n.nodes.get(n.children[i].tag))
                  n.nodes.set(n.children[i].tag, n.children[i]);
                if (!(n.children[i].tag in n))
                  n[n.children[i].tag] = n.children[i].tag;
                ...
                if (n.graph && n.graph.get(n.children[i])) {
                  n.children[i] = n.graph.get(n.children[i]);
                  if (!n.nodes.get(n.children[i].tag))
                    n.nodes.set(n.children[i].tag, n.children[i]);
                  if (!(n.children[i].tag in n))
                    n[n.children[i].tag] = n.children[i].tag;
                }
                if (!n.children[i] && n.nodes.get(n.children[i]))
                  n.children[i] = n.nodes.get(n.children[i]);
                checkParentHasChildMapped(n, n.children[i]);
              }
            }
            ...
            n.nodes.set(n.children.tag, n.children);
            if (!(n.children.tag in n))
              n[n.children.tag] = n.children.tag;
            checkParentHasChildMapped(n, n.children);
            n.convertChildrenToNodes(n.children);
          } else if (typeof n.children === "string") {
            if (n.graph && n.graph.get(n.children)) {
              n.children = n.graph.get(n.children);
              if (!n.nodes.get(n.children.tag))
                n.nodes.set(n.children.tag, n.children);
              if (!n[n.children.tag])
                n[n.children.tag] = n.children.tag;
            }
            if (!n.children && n.nodes.get(n.children))
              n.children = n.nodes.get(n.children);
            checkParentHasChildMapped(n, n.children);
          }
        }
        return n.children;
      ...
      if (typeof properties === "object") {
        ...
        if (parentNode && !properties.parent)
        ...
        if (!graph && properties.oncreate)
          properties.oncreate(this);
      } else
        return properties;
    }
  };
    ...
    constructor(tree, tag, props) {
      ...
      this.setTree = (tree = this.tree) => {
        ...
        this.nodes.forEach((node) => {
          let checkParentHasChildMapped = (node2, child) => {
            if (node2.parent instanceof GraphNode || node2.parent instanceof Graph) {
              if (!node2.parent.nodes.get(child.tag))
                node2.parent.nodes.set(child.tag, child);
              if (node2.parent.parent) {
                checkParentHasChildMapped(node2.parent, child);
              }
            }
          };
          if (typeof node.children === "string") {
            ...
            node.children.forEach((c, i) => {
              if (typeof c === "string") {
                if (this.nodes.get(c)) {
                  node.children[i] = this.nodes.get(c);
                  node.nodes.set(node.children[i].tag, node.children[i]);
    ...
    constructor(options2) {
      ...
      this.load = (routes, enumRoutes = true) => {
        ...
        for (const route in routes) {
          let childrenIter = (route2) => {
            if (route2?.children) {
              if (Array.isArray(route2.children)) {
                route2.children.forEach((c) => {
                  if (typeof c === "object") {
                    if (c.tag) {
                      routes[c.tag] = c;
                    } else if (c.id) {
              ...
              } else if (typeof route2.children === "object") {
                if (route2.children.tag) {
                  routes[route2.children.tag] = route2.children;
                } else if (route2.children.id) {
                  route2.children.tag = route2.children.id;
                  routes[route2.children.tag] = route2.children;
                  childrenIter(routes[route2.children.tag]);
                }
              }
            }
          };
