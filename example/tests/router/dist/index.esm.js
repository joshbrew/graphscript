  ...
  constructor(properties = {}, parentNode, graph2) {
    ...
    if (typeof properties === "object") {
      ...
      if (this.parent instanceof GraphNode)
        checkParentHasChildMapped(this.parent, this);
  ...
  constructor(tree2, tag, props) {
    ...
    this.setTree = (tree2 = this.tree) => {
      ...
      this.nodes.forEach((node) => {
        let checkParentHasChildMapped = (node2, child, depth = 0) => {
          if (node2.parent instanceof GraphNode || node2.parent instanceof Graph) {
            ...
            if (node2.parent.parent && depth < this.nodes?.size) {
              checkParentHasChildMapped(node2.parent, child, depth++);
            }
          }
        };
        if (typeof node.children === "object") {
    ...
    this.load = (routes2, enumRoutes = true) => {
      ...
      for (const route in routes2) {
        let childrenIter = (route2) => {
          if (typeof route2?.children === "object") {
            for (const key in route2.children) {
              if (typeof route2.children[key] === "object") {
                let rt = route2.children[key];
                if (rt.tag) {
                  routes2[rt.tag] = route2.children[key];
